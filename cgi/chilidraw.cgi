...
my $query = new CGI;
my $al=$query->param('ALG');
my $symb=$query->param('SYM');
my $depth=$query->param('DEP');
my $folder=$query->param('FLD');
my $min_snt=$query->param('SNT');
my $percent=$query->param('PCT');
#my $level5=$query->param('level5');
#my $level5=uc($level5);
my $project="/home/httpd/html/chilibot/$user/$folder";
...
my @select= grep {/\"$symb\_|\_$symb\"/} @old; # /"symb='edge' while dealing with the entire graph

#get node color
my %color;
my @nodes=grep {/node/} @old;
...
#		next if ($key eq $symb);
	...
	foreach(@old) { #decide which one not to print
		...
		if ($_ =~ /edge/) {
			...
			$weight=0 if ($_=~/label: *\"=\"/); # parallel
			$weight=-2 if ($_=~/label: *\"&\"/); # abstract only
		} else {
			next;
		}
		if ($name=~/\_/){
			...
			foreach (@three) {
#				$skip=1 if (!$node{$_});
				$skip=1 if ($weight<=$min_snt);
#				print "depth $depth// $name lnkwght  $weight, usersele: $min_snt, skip? $skip<br>"; 
			}
		} 
		#	else {
#			$skip=1 if (!$node{$name});
...
print "<center><h3>$folder</h3>Showing only strong interactive relationships |" if ($depth ==2 and $min_snt==5);
print "<center><h3>$folder</h3>Showing only interactive relationships |" if ($depth ==2 and $min_snt==0);
print "<center><h3>$folder</h3>Excluding relationship only found at the abstract level|" if ($depth ==2 and $min_snt==-1);
